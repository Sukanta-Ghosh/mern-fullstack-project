/* Clousre Function example */
// const createResourceFactory = (model) => {
//     console.log("ran resource Model");
//     return function () {
//         console.log(`Creating a new  resource using ${model}`);
//     }
// }

// const createUserHandler = createResourceFactory("userModel");
// const createProductHandler = createResourceFactory("productModel");
...
const getAllResourceFactory = (model) => {
  return async function getAllProducts(req, res) {
    try {
      ...
      let limit = queryObject.limit || 10;
      /****
      ...
      query = query.skip(elementsToSkip).limit(limit);
...
const updateResourceByIdFactory = (model) => {
  return async function updateProductById(req, res) {
    try {
       ...
       * **/
      const product = await model.findByIdAndUpdate(id, toUpdateObject, {
        new: true,
      });
      if (product) {
        res.status(200).json({
